...
task :start_test_server, :port do |t, args|
    puts "starting server on http://localhost:#{args.port}"
    sh "java -jar /usr/bin/jstestdriver/JsTestDriver-1.2.2.jar --port #{args.port}"
end

task :run_tests do
    puts "running tests..."
    sh "java -jar /usr/bin/jstestdriver/JsTestDriver-1.2.2.jar --tests all --reset"
end

task :unit_tests do
    puts 'running unit tests'
    sh 'java -jar /usr/bin/jstestdriver/JsTestDriver-1.2.2.jar --port 3232 --browser $FIREFOX --tests all --reset' do |ok, res|
        if not ok
            return false
        end
    end
